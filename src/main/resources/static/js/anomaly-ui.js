(() => {
    ...
    function render(rows) {
        ...
        if (body) {
            ...
            document.getElementById('anomaly-count').textContent = `누적 이상 감지 ${rows.length}건`;
            if (!rows.length) {
            ...
            rows.forEach(row => {
                const tr = el('tr');
                tr.append(el('td', new Date(row.createdAt).toLocaleString('ko-KR')), el('td', row.drugName), el('td', `${description(row)} / 전표 ${row.receiptId}`), el('td', '수량 불일치'));
                const status = el('td');
